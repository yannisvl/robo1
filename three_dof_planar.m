...
rd2 = xd(:).^2 + yd(:).^2 + zd(:).^2; 
qd(:,3) = acos( (rd2(:)-l(2)^2-l(3)^2-l(1)^2)./(2*l(2)*l(3)) ); 	
...
c2 = cos(qd(:,2));
c23 = cos(qd(:,2)+qd(:,3));
qd(:,1) = 2*atan((l(3)*c23 + l(2)*c2 - sqrt((l(2)*c23+l(2)*c2).^2+l(1)^2-xd(:).^2)) ./ (xd(:) + l(1)) );
...
xd2 = l(1)*cos( qd(:,1) ) + l(2)*cos( qd(:,2) ).*sin( qd(:,1) );   
yd2 = l(2)*sin ( qd(:,2) );
zd2 = l(1)*sin( qd(:,1) ) - l(2)*cos( qd(:,1) ).*cos( qd(:,2) );
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%%(xd3, yd3, zd3) : cartesian position of the endpoint
xd3 = l(3)*sin( qd(:,1) ).*cos( qd(:,2)+qd(:,3) ) + l(2)*sin( qd(:,1) ).*cos( qd(:,2) ) + l(1)*cos (qd(:,1) );
yd3 = l(3)*sin( qd(:,2)+qd(:,3) ) + l(2)*sin( qd(:,2) );
zd3 = -l(3)*cos( qd(:,1) ).*cos( qd(:,2)+qd(:,3) ) -  l(2)*cos( qd(:,1) ).*cos( qd(:,2) ) + l(1)*sin (qd(:,1) );
...
fig3 = figure; 
axis([-5 25 -15 15]) %%set xyz plot axes (caution: square axes, i.e. dx=dy=dz) 
axis on 
hold on 
xlabel('x (cm)'); 
ylabel('y (cm)'); 
